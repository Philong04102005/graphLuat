...
def extract_hover_content(soup: BeautifulSoup, element) -> str:
    ...
    print(f"   🛠️  Xử lý hover: class={tooltip_class}, found_div={bool(tooltip_div)}")

    if tooltip_div:
        divs = tooltip_div.find_all("div")

        texts = []

        if divs:
            for div in divs:
                tooltip_text = div.get_text(separator=' ', strip=True)
                tooltip_text = re.sub(r'\s+', ' ', tooltip_text).strip()

                if tooltip_text and tooltip_text != "Click vào để xem nội dung":
                    if tooltip_text not in texts:
...
def render_tooltips_fast(page, chunk_scroll=250, wait_ms=800):
    """
    Render toàn bộ tooltip/hover trong nội dung văn bản.
    Bản này quét nhiều container hơn và cập nhật lại chiều cao trang liên tục.
    """
    page.evaluate("""
    ...
    while True:
        ...
        if y >= total_height:
            ...
            y = max(0, total_height - viewport_height)
        ...
        const selector = [
            "div.content1 [onmouseover]",
            "div.content1 [atmm]",
            "div.content2 [onmouseover]",
            "div.content2 [atmm]",
            "#toanvan [onmouseover]",
            "#toanvan [atmm]",
            ".fulltext [onmouseover]",
            ".fulltext [atmm]"
        ].join(",");
        ...
        const els = [...document.querySelectorAll(selector)]
            .filter(el => {
                ...
                return r.top >= -500 && r.top <= window.innerHeight + 500;
            });
